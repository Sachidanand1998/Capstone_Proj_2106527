package com.capstone.project.movieappbackend.filter;
...
public class AppFilter extends OncePerRequestFilter {
    ...
    private static final List<String> AUTH_WHITELIST = Arrays.asList("/api/v1/user/save", "/api/v1/user/login", "/api/v1/movies");
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain)
        ...
        try {
            Claims claims = Jwts.parser().setSigningKey("CAPSTONE-MOVIEAPP").parseClaimsJws(token.replace("Bearer ", "")).getBody();
